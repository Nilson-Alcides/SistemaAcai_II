    ...
    public class ExportaArquivo
    {
        public byte[] GerarExcel(List<Comanda> comandas)
        {
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 5).Value = "Desconto";
                ...
                for (int i = 0; i < comandas.Count; i++)
                {
                    ...
                    worksheet.Cell(i + 2, 5).Value = c.Desconto;
                    worksheet.Cell(i + 2, 6).Value = c.ValorTotal;
                }

                // Linha de total
                int linhaTotal = comandas.Count + 2;
                worksheet.Cell(linhaTotal, 1).Value = "TOTAL";
                worksheet.Range(linhaTotal, 1, linhaTotal, 5).Merge().Style
                    .Alignment.SetHorizontal(XLAlignmentHorizontalValues.Right)
                    .Font.SetBold(true)
                    .Fill.SetBackgroundColor(XLColor.LightGray);

                worksheet.Cell(linhaTotal, 6).Value = comandas.Sum(c => c.ValorTotal);
                worksheet.Cell(linhaTotal, 6).Style.Font.SetBold(true);
        ...
        public byte[] GerarPdf(List<Comanda> comandas)
        {
            using (var stream = new MemoryStream())
            {
                ...
                string[] headers = { "ID", "Cliente", "Data Abertura", "Data Fechamento", "Desconto", "Valor Total" };
